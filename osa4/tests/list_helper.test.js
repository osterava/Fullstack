...
describe('Favorite blog post', () => {
  test('when list has only one blog, it should return that blog', () => {
    const result = listHelper.favoritBlog([blogs[0]]);
    expect(result).toEqual(blogs[0])
  })

  test('when list is empty, it should return null', () => {
    const blogs = [];
    const result = listHelper.favoritBlog(blogs);

    expect(result).toEqual();
  })

  test('when list has multiple blogs, it should return the one with the most likes', () => {
    const result = listHelper.favoritBlog(blogs);
